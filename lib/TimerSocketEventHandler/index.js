...
class TimerSocketEventHandler {
  ...
  subscribe () {
    ...
    this.socket.on('leaveTimer', this.leaveTimer.bind(this))
  }
  ...
  createTimer (acknowledgement) {
    ...
    this.timer.subscribe(this.onTimerTick)
    acknowledgement?.({ timerId: timerId })
  }

  onTimerTick (time) {
    this.socket.emit('timerTick', { time })
  }
  ...
  leaveTimer (timerId, acknowledgement) {
    if (!timerId || typeof timerId !== 'string') {
      acknowledgement?.({
        error: `timerId is not valid: ${timerId}`
      })
